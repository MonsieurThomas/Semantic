const Prompt2 = `En tant qu’élément complémentaire de l’algorithme de mon logiciel qui, à partir d’un texte (lui-même pouvant regrouper plusieurs textes de différents auteurs) renvoie une mind map qui, à l’extrémité de ses branches renvoie à un bloc spécifique du texte total, j’ai besoin que tu me rédiges l’ensemble de la structure de cette mind map via des mots-clés que tu vas associés à chacun des groupes de blocs de texte, et les hiérarchiser dans la mind map.  
    ...
    5.1. Itération des étapes 3 et 4 : les groupes se rassemblent à nouveau (par proximité de sens sémantique) par groupe pour arriver à construire l’avant-avant-dernier niveau de la mind map, en veillant à respecter les critères de nombre de blocs par groupe. 
    5.2. Répète ce processus jusqu’à l’obtention des branches dites primaires, au premier niveau de profondeur de la mind map.
    
    6. Donner un titre à la map
    6.1Finalise la mind map en donnant un titre reflétant le contenu global du texte et le nom des auteurs.
    ...
    1. Pour que mon logiciel puisse retranscrive la mind map, il faut qu’absolument tous les blocs de texte que tu as découpés soient aux branches les plus éloignés de la mind map. Le nom des blocs ne doivent être mentionnés qu’au “dernier niveau de branches”, jamais avant.
    ...
    3. Veuillez créer une mind map hiérarchisée avec plusieurs niveaux de profondeur, où chaque niveau décompose davantage les détails du niveau précédent. Assurez-vous d'extraire et de regrouper les informations de manière à ce que chaque branche, sous-branche (sous-sous branche, etc.) de la mind map reflète clairement un concept ou une idée spécifique provenant du texte, permettant ainsi une analyse approfondie des relations et des sous-thèmes contenus dans le texte original.
    
    4. Ta réponse ne doit contenir que la structure JSON correspondant à la mind map du texte sans aucune introduction ou commentaire supplémentaire, ni interruption. Utilise exclusivement des accolades pour imbriquer les éléments, et n'inclus pas de crochets pour les listes ou les arrays. Chaque titre de section de la mind map doit être une clé dans l'objet JSON, et chaque sous-section (ou sous-sous-section, etc.) doit être un objet imbriqué correspondant.
    
    5. Pour garantir un traitement systématique et cohérent du texte en un organigramme conforme, je demande une réponse au format JSON. Le format des entrées doit être le suivant "Bloc (numéro du bloc)" suivi de la citation textuelle, débutant par les 5 premiers mots et se terminant par les 5 derniers mots du bloc.
    ...
    Voici un exemple de la structure désirée :
    {
      "Titre Global": {
        "Catégorie Principale 1": {
          "Sous-Catégorie A": {
            "Bloc 1.1": "Début de citation... —> ...fin de citation."
          },
          "Sous-Catégorie B": {
            "Bloc 1.2 ": "Début de citation... —> ...fin de citation."
          }
        },
    ...
    Tu peux créer plus d'imbrication pour avoir une plus grande profondeur dans la mindmap, par exemple avoir une sous-sous caégorie A.1 avec des blocs 1.1.1 et 1.1.2. Le nombre total de niveau de profondeur dépendra du texte à analyser.
    ...
    En élaborant le format JSON pour chaque partie de texte, idnetifie les les débuts et fins de citations. Si ces mots apparaissent ailleurs dans le texte original exactement de la même manière, ajoute une précision en donnant les mots qui les précèdent directement et/ou qui les suivent directement pour éviter toute ambiguïté. Si aucun des mots ne se répète de manière identique dans le texte original, il n'est pas nécessaire de spécifier de mots supplémentaires.
...
const Prompt = `En tant qu’élément complémentaire de l’algorithme de mon logiciel qui, à partir d’un texte (lui-même pouvant regrouper plusieurs textes de différents auteurs) renvoie une mind map qui, à l’extrémité de ses branches renvoie à un bloc spécifique du texte total, j’ai besoin que tu me rédiges l’ensemble de la structure de cette mind map via des mots-clés que tu vas associés à chacun des groupes de blocs de texte, et les hiérarchiser dans la mind map.  
    ...
    Voici un exemple de la structure désirée :
    {
      "Titre Global": {
        "Catégorie Principale 1": {
          "Sous-Catégorie A": {
            value: "Début de citation... —> ...fin de citation."
          },
          "Sous-Catégorie B": {
            value: "Début de citation... —> ...fin de citation."
          }
